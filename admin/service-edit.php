...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
        // Insert the new service into the database first
        $statement = $pdo->prepare("INSERT INTO tbl_service (title, content) VALUES (?, ?)");
        $statement->execute(array($_POST['title'], $_POST['content']));

        // Get the auto_increment ID of the newly inserted record
        $ai_id = $pdo->lastInsertId();

        // Prepare the final file name and move the uploaded file
